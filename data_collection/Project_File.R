...
model_data <- model_data%>%
  mutate(profit_margin = round(profit_margin,5),
         ...
         debt_ratio = round(debt_ratio, 5),
         debt_equity = round(debt_equity, 5),
         interest_cov = round(interest_cov, 5),
         gross_margin = round(gross_margin, 5),
         roa = round(roa, 5),
         roe = round(roe, 5))
  


# Final shrinkage to only past two quarters, lag variables, final processing of data set:



# CURRENT QUARTER MODEL DATASET
